...
For working with vector data we will use, among others `GeoPandas` and for vector data we will use `Rasterio` for raster data. During these examples we will import and use these packages to load the data. A more elaborate introduction to these packages will follow in the respective Tutorials covering raster and vector analysis. 
...
## Cartopy
As said, Cartopy is basically adding the spatial component to matplotlib. Therefor a lot of the logic will be familiar. By addding tpatial information, a coordinate reference system, to an Axes (turning it into a GeoAxes) we can reference our spatial data to each other. Cartopy hsa abuilt in module to handle the referencing `cartopy.crs`. Let's import this. 
...
Let's start by creating a map of the world. We do this by generating 1 subplot (so just a plot) with the PlateCarree projection, a projection where every poitn is spaced out equally in terms of degrees. 
