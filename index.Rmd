...
pagetitle: "Week 3, Lesson 8: Introduction to Python for Geo-Scripting"
output:
...
This will install Miniconda into `~/miniconda3`. Finally, restart your terminal to be able to use `conda` in the terminal. Next, let's see how to use Conda in case you want make new virtual environments by yourself, or install packages after creating the environment.
...
The [Spyder IDE](https://docs.spyder-ide.org/) can be started in a terminal when the spyder package is installed in the active conda environment. So make an environment, install Spyder, and activate the environment. Spyder automatically uses the Python interpreter of the active conda environment. Start Spyder:
...
Next, we will create a conda environment from a file. Hereto, create a text file. This can be done, for example, with the `cat` command.
```{r, eval=FALSE,engine='bash'}
cat > file1.txt
```

By right-clicking the new file, (re)name it (to) `refresher.yaml`, and copy the following content into the file:
...
```{r, eval=FALSE, engine='bash'}
conda env create --f refresher.yaml
```
...
Create a new Python script and save it in the `scripts` folder. 
...
<img src="images/numpy_structure.png" alt="matplotlib class structure" width="100%"></img>
...
<img src="images/numpy_slicing.png" alt="matplotlib class structure" width="50%"></img>
...
<img src="images/mpl_simple_plot.png" alt="matplotlib simple plot" width="50%"></img>
...
1. Python package management with conda
