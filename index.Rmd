...
Your Jupyter Notebook is automatically saved as an .ipynb file on your computer (the file extension comes from the historic name "IPython Notebook"). The notebook can be downloaded as a Python script, pdf or html. You can also save it manually. To exit a notebook properly, use *File* → *Close and Halt*.
...
For example, one could have a class `dog` with functions, `eat()` and `run()`, and properties `breed` and `age`. One can then create an *instance* of this class, i.e. an object, that represents a Maltese of two years old. This object can apply the function `eat()` and `run()`.
...
For example, we could create a function to multiply a number (provided as argument) by three, and return the output and input number: 
...
Variables created in functions are *local*. That means that a variable created in a function does not exist outside of the function. So, here, `output_number` cannot be accessed outside of the function definition (after the indented part of the code). 


```{block, type="alert alert-success"}
> **Question 3**: Test typing `print(output_number)` at the end of the code block defined above. What happens? Why is that?
```
...
```{block, type="alert alert-success"}
> **Question 4**: What are the differences between these four ways to import modules and/or packages?
```
...
```{block, type="alert alert-success"}
> **Question 6:** What are the major differences between Append/Extend?
```

```{block, type="alert alert-success"}
> **Question 7:** What building is `campus[-2]`?
```
...
```{block, type="alert alert-success"}
> **Question 8**: What does `w`, `r` and `rw` mean?
```
