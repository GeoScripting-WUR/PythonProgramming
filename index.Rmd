...
* Spyder is nice a lightweight IDE and can be installed from the terminal. In the Geo-Scripting course Spyder is recommended as Python IDE.
* Sublime Text is a modern text editor that does nice Python highlighting.
* Jupyter notebook or Ipython Notebook are good choices for short scripts and integrate visualizations together with the code. See here for a simple Jupyter [notebook example](http://nbviewer.ipython.org/github/GeoScripting-WUR/PythonWeek/blob/gh-pages/A%20simple%20notebook.ipynb). [More info](http://jupyter.org/).
* [PyCharm](https://www.jetbrains.com/pycharm/) is a free professional Python IDE with a lot of advanced functionality, such as integrated GIT version control.
...
* Using [conda](https://conda.io/docs/user-guide/tasks/manage-pkgs.html) to install packages and to keep separate sets of packages. Conda is available for Windows, macOS and Linux.
...
* Using the distribution's package manager (on Ubuntu, that's `sudo apt-get install python-*`).
...
- Type the following to start Python 3 in the terminal:

```{r, eval=FALSE, engine='bash'}
python3
```

An example script to find out what the installed Python version is.
