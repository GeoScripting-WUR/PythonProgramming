...
```{r, eval=FALSE, engine='bash'}
# Cross-platform (but not always working, like in our VDI, so we use the next option)
...
```{block, type="alert alert-success"}
> **Question 9**: In the upper right subplot, why is there no point at x=3, y=8?.
```
...
```{block, type="alert alert-success"}
> **Question 10**: What kind of functionality does the `sys` package provide?
```
