...
```{r, eval=FALSE, engine='bash'}
mamba create --name geotest python=2.7 numpy
```

This would create a new environment called `geotest` with Python 2.7 and `NumPy` installed into the `conda` environment. Another option is to create an environment from a `.yaml` file, in which all required modules are listed. You will see an example of this later on in this tutorial. To create an environment from such a file, you can use the argument `--file` (or `--f` in short). 
...
The `mamba` command searches a set of channels. By default, packages are automatically downloaded and updated from the default channel. To search for a package, type:
...
This installs the latest version of the _Spyder_ package (Python IDE). Note that this would install it into your currently activated environment.

Note that you can also install multiple packages at the same time: 

```{r, eval=FALSE, engine='bash'}
mamba install geopandas pandas
```
...
```{r, eval=FALSE, engine='bash'}
mamba install --channel conda-forge folium
```

As explained above, another alternative is to use `pip` to install packages (not possible for all packages), as shown here for the packages `rasterio` and `rasterstats`:

```{r, eval=FALSE, engine='bash'}
pip install rasterio rasterstats
```

In addition, as you saw with _Spyder_ install, `mamba` is able to install some non-Python packages that have Python bindings, such as _Spyder_ and _GDAL_. This is useful for making sure your Python and binary versions match and do not interfere with the system-wide ones.
